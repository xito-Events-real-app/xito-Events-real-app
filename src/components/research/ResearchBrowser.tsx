 ...
 * Quotation, Payments, Project Managers, Lightroom Catalog.
 */
export function ResearchBrowser({ clients, assignments, isLoading }: Props) {
  ...
  const virtualFolderNames = useMemo(() => {
    ...
    if (currentLevel === 0) {
      ...
      if (selectedCategory === "Project Managers" || selectedCategory === "Lightroom Catalog") {
        currentClientFolder.events.forEach(e => names.add(e));
      }
    } else if (currentLevel === 4 && currentClientFolder && selectedCategory === "Lightroom Catalog") {
  ...
  const extraPCloudFolders = useMemo(() => {
    return pcloudItems.filter(item => item.isfolder && !virtualFolderNames.has(item.name));
  ...
  const renderContent = () => {
      ...
      return (
        <div className={gridClass}>
          {currentGroup.clients.map(c => (
            <XitoDriveFolderCard key={c.registeredDateTimeAD} name={c.clientName} itemCount={RESEARCH_CATEGORIES.length} type="client" onClick={() => navigate(c.clientName, c.registeredDateTimeAD)} />
          ))}
    ...
    if (currentLevel === 3 && currentClientFolder) {
        ...
        return (
          <div className={gridClass}>
            {currentClientFolder.events.map(ev => (
              <XitoDriveFolderCard key={ev} name={ev} type="event" categoryName="Project Managers" onClick={() => navigate(ev, ev)} />
            ))}
            {extraPCloudFolders.map(f => (
              <XitoDriveFolderCard key={f.name} name={f.name} type="leaf" onClick={() => navigate(f.name, f.name)} />
            ))}
      ...
      if (selectedCategory === "Lightroom Catalog") {
        return (
          <div className={gridClass}>
            {currentClientFolder.events.map(ev => {
              const { photographers } = getFreelancersForEvent(assignments, currentClientFolder.registeredDateTimeAD, ev);
              return (
                <XitoDriveFolderCard key={ev} name={ev} itemCount={photographers.length || undefined} type="event" categoryName="Lightroom Catalog" onClick={() => navigate(ev, ev)} />
              );
            })}
            {extraPCloudFolders.map(f => (
              <XitoDriveFolderCard key={f.name} name={f.name} type="leaf" onClick={() => navigate(f.name, f.name)} />
    ...
    // Level 4: Photographers under Lightroom Catalog > Event
    if (currentLevel === 4 && currentClientFolder && selectedCategory === "Lightroom Catalog") {
      const { photographers } = getFreelancersForEvent(assignments, currentClientFolder.registeredDateTimeAD, selectedEvent!);
      return (
        <div className={gridClass}>
