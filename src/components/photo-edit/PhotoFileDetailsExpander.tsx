...
const PHOTO_ROLE_ORDER: Record<string, number> = {
  PB: 1, PG: 2, EP: 3,
};

function isPhotoRole(code: string) {
  return PHOTO_ROLES.includes((code || "").toUpperCase());
}

export function PhotoFileDetailsExpander({ registeredDateTimeAD, eventName, compact }: Props) {
  ...
  // Filter to photo roles only
  const files = useMemo(
    () => allFiles.filter(f => isPhotoRole(f.freelancer_type || "")),
    [allFiles]
  );

  const sortedFiles = useMemo(() => {
    return [...files].sort((a, b) => {
      const oa = PHOTO_ROLE_ORDER[(a.freelancer_type || "").toUpperCase()] || 50;
      const ob = PHOTO_ROLE_ORDER[(b.freelancer_type || "").toUpperCase()] || 50;
  ...
  const photographerNames = useMemo(() => {
    ...
    files.forEach(f => { if (f.freelancer_name) set.add(f.freelancer_name); });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 py-3 px-4">
        ...
        <span className="text-xs text-muted-foreground">Loading photo files...</span>
      </div>
    );
  }
  ...
  if (files.length === 0) {
    return (
      <div className="py-3 px-4">
        <span className="text-xs text-muted-foreground italic">No photo file records found for this event</span>
      </div>
    );
  }
    ...
    <div className={`space-y-2.5 ${compact ? 'px-2 py-2' : 'px-4 py-3'}`}>
      ...
      <div className="flex flex-wrap items-center gap-2.5 text-xs rounded-lg bg-accent/40 border border-border/60 px-3 py-2">
        ...
        <span className="font-semibold text-foreground">
          Photo Size: <span className="text-primary font-bold">{totalSizeLabel}</span>
        </span>
        ...
        {allCopied ? (
          <span className="font-bold text-green-600 dark:text-green-400 flex items-center gap-1">
            <CheckCircle2 className="w-3.5 h-3.5" /> ALL PHOTO FILES COPIED
          </span>
        ) : (
