...
export function AddTaskDrawer({ open, onOpenChange, setupData, onTaskAdded }: AddTaskDrawerProps) {
  const todayStr = format(new Date(), "yyyy-MM-dd");
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    dateAD: todayStr,
    dateBS: convertToBS(todayStr),
    taskName: "",
    description: "",
    deadline: "",
    handler: "",
  ...
  const updateField = (field: string, value: string | number) => {
    setForm(prev => {
      ...
      if (field === "dateAD" && typeof value === "string") {
        updated.dateBS = convertToBS(value);
      }
      return updated;
    });
  };
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-[440px] sm:max-w-[440px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="text-lg font-bold">Add New Task</SheetTitle>
        </SheetHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                type="date"
                value={form.dateAD}
                onChange={e => updateField("dateAD", e.target.value)}
                className="text-sm"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="mt-2 px-1">
              <Slider
                value={[form.urgency]}
                onValueChange={v => updateField("urgency", v[0])}
                min={1}
                max={5}
                step={1}
                className="w-full"
