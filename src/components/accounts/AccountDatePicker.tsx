...
export function AccountDatePicker({ 
  selectedDate, 
  onDateChange, 
  ...
  const [mode, setMode] = useState<'ad' | 'bs'>('bs'); // Default to BS
  ...
  const getDisplayText = () => {
    ...
    const adFormatted = format(selectedDate, "yyyy-MM-dd");
      ...
      <PopoverContent className="w-auto p-0 bg-slate-900 border-slate-700" align="start">
        <div className="p-3 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-slate-300 text-sm font-medium">Select Date</span>
            ...
            {mode === 'ad' ? (
              ...
              <Calendar
                ...
                classNames={{
                  ...
                  cell: "h-8 w-8 text-center text-sm p-0 relative flex-1 [&:has([aria-selected])]:bg-pink-600/20 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
                  day: cn(
                    "h-8 w-8 p-0 font-normal mx-auto rounded-md hover:bg-slate-700 text-slate-200 inline-flex items-center justify-center"
                  ),
                  day_selected:
                    "bg-pink-600 text-white hover:bg-pink-600 hover:text-white focus:bg-pink-600 focus:text-white",
                  day_today: "bg-slate-700 text-pink-400",
