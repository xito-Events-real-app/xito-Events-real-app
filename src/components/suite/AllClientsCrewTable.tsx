...
import { Loader2, Users, Plus, X, ChevronLeft, ChevronRight, Database, Trash2, Download, Upload, Cloud, ChevronDown, ChevronUp, Phone, MapPin, StickyNote, Pencil, Filter, ArrowUpDown, MessageCircle, Send } from "lucide-react";
...
function EventLogisticsPanel({ eventDetail, contactDetail, settings: settingsProp, loading, row }: {
  ...
  const mappedEventDetail = useMemo<EventDetail | undefined>(() => {
    ...
    return {
      ...
      brideStartTime: eventDetail.bride_start_time || '',
      brideEndTime: eventDetail.bride_end_time || '',
      groomStartTime: eventDetail.groom_start_time || '',
      groomEndTime: eventDetail.groom_end_time || '',
...
function FreelancerHoverInfo({ name, allAssignments, selectedYear, selectedMonth, freelancers, onFilterFreelancer }: { name: string; allAssignments: FreelancerAssignment[]; selectedYear: string; selectedMonth: string; freelancers: FreelancerData[]; onFilterFreelancer?: (name: string) => void }) {
  ...
  const { monthEvents, upcomingEvents, completedCount } = useMemo(() => {
    ...
    const completed = monthEvts.filter(ev => isBSDatePast(ev.year, ev.month, ev.day)).length;
    const upcoming = monthEvts.filter(ev => !isBSDatePast(ev.year, ev.month, ev.day));
    upcoming.sort((a, b) => (parseInt(a.day) || 0) - (parseInt(b.day) || 0));

    return { monthEvents: monthEvts, upcomingEvents: upcoming, completedCount: completed };
  ...
  const handleChatWhatsApp = () => {
    if (!phone) { toast.error(`No phone number found for ${name}`); return; }
    openWhatsApp(phone);
  };

  const handleSendSchedule = () => {
    if (!phone) { toast.error(`No phone number found for ${name}`); return; }
    openWhatsApp(phone, scheduleMessage);
      ...
      <div className="space-y-3 min-w-[240px]">
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="font-bold text-sm text-gray-900 hover:text-violet-600 transition-colors"
          >
            {name}
          </button>
        ...
        {/* Stats Row */}
        <div className="flex items-center gap-2">
          <div className="flex-1 text-center bg-violet-50 rounded-lg py-1.5 px-2">
            <p className="text-sm font-black text-violet-700">{total}</p>
            <p className="text-[10px] font-medium text-violet-500">Total</p>
          </div>
          <div className="flex-1 text-center bg-amber-50 rounded-lg py-1.5 px-2">
            ...
            <p className="text-[10px] font-medium text-amber-500">Remaining</p>
          </div>
          <div className="flex-1 text-center bg-emerald-50 rounded-lg py-1.5 px-2">
        ...
        {upcomingEvents.length > 0 && (
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Upcoming</p>
            <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
              {upcomingEvents.map((ev, i) => (
                <div key={i} className="text-xs border-l-2 border-violet-300 pl-2 py-0.5">
                  <span className="font-bold text-violet-700">{ev.day} {nepaliMonthsEnglish[parseInt(ev.month) - 1] || ev.month}</span>
                  <span className="text-gray-500 ml-1">— {ev.clientName} • {ev.event}</span>
                </div>
              ))}
            </div>
          </div>
        )}
        {upcomingEvents.length === 0 && total > 0 && (
          <p className="text-xs text-gray-400 italic">All events completed</p>
        )}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold py-1.5 px-2 rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold py-1.5 px-2 rounded-lg transition-colors"
            >
          ...
          <DialogContent className="max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-gray-500">
                This will open WhatsApp with the schedule link.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex gap-2 mt-2">
              ...
              <button
                ...
                className="flex-1 py-2 text-xs font-semibold text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg transition-colors flex items-center justify-center gap-1"
              >
                <Send className="w-3 h-3" />
                Send via WhatsApp
              </button>
