...
const EventAssignmentCard = ({ assignment, freelancers, isUpdating, onUpdate, onCheckAvailability, registeredDateTimeAD, settings, onUpsertSetting, eventDemands, refetch }: EventAssignmentCardProps) => {
  ...
  // Fetch contact details for the CrewScheduleEventSheet preview
  const { data: contactDetails } = useClientContactDetails(registeredDateTimeAD);
  const { data: eventDetailsData } = useEventDetails(registeredDateTimeAD);

  // Build freelancer phone map from freelancers list
  const freelancerPhones = useMemo(() => {
    const map = new Map<string, string>();
    for (const fl of freelancers) {
      if (fl.name && fl.contactNo) {
        map.set(fl.name.toLowerCase(), fl.contactNo);
      }
    }
    return map;
  }, [freelancers]);

  // Map assignment to AssignmentRow format for CrewScheduleEventSheet
  const mappedAssignmentRow = useMemo((): AssignmentRow => ({
    event_year: assignment.eventYear || null,
    event_month: assignment.eventMonth || null,
    event_day: assignment.eventDay || null,
    event: assignment.event || '',
    client_name: assignment.clientName || null,
    registered_date_time_ad: registeredDateTimeAD,
    photographer_bride: assignment.photographerBride || null,
    photographer_groom: assignment.photographerGroom || null,
    videographer_bride: assignment.videographerBride || null,
    videographer_groom: assignment.videographerGroom || null,
    extra_photographer: assignment.extraPhotographer || null,
    extra_videographer: assignment.extraVideographer || null,
    assistant: assignment.assistant || null,
    iphone_shooter: assignment.iphoneShooter || null,
    drone_operator: assignment.droneOperator || null,
    fpv_operator: assignment.fpvOperator || null,
  }), [assignment, registeredDateTimeAD]);

  // Find the matching EventDetail for the CrewScheduleEventSheet
  const matchedEventDetail = useMemo(() => {
    return eventDetailsData?.events?.find(ed =>
      ed.eventName?.toLowerCase() === assignment.event?.toLowerCase()
    );
  }, [eventDetailsData, assignment.event]);
  ...
  const handleOpenCalendar = useCallback((freelancerName: string) => {
  ...
  return (
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
      ...
      {/* Freelancer Event Preview Sheet */}
      {calendarOpenFor && (
        <CrewScheduleEventSheet
          open={!!calendarOpenFor}
          onOpenChange={(open) => { if (!open) setCalendarOpenFor(null); }}
          assignment={mappedAssignmentRow}
          eventDetail={matchedEventDetail}
          contactDetails={contactDetails}
          freelancerPhones={freelancerPhones}
          freelancerName={calendarOpenFor}
        />
      )}
    </div>
  );
