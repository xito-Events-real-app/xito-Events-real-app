...
import { ChevronDown, ChevronUp, MapPin, Clock, Users, FileText, Link2, Plus, Trash2, Loader2, AlertTriangle, Save } from 'lucide-react';
...
import { EventDetail, serializeQuotedList } from '@/hooks/useEventDetails';
...
export function EventDetailCard({ event, eventDateAD, onSave, isLoading = false }: EventDetailCardProps) {
  ...
  const isUrgent = isEventUrgent(eventDateAD || '');
  const hasEmptyFields = hasEmptyRequiredFields(event);
  const showUrgencyWarning = isUrgent && hasEmptyFields;
  ...
  const formattedDate = `${monthName} ${event.eventDay || '?'}, ${event.eventYear || ''}`;
      ...
      >
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            event.eventName.toUpperCase().includes('WEDDING') ? "bg-blue-500/30 text-blue-200" :
            event.eventName.toUpperCase().includes('RECEPTION') ? "bg-purple-500/30 text-purple-200" :
            event.eventName.toUpperCase().includes('ENGAGEMENT') ? "bg-pink-500/30 text-pink-200" :
            event.eventName.toUpperCase().includes('PRE') ? "bg-orange-500/30 text-orange-200" :
