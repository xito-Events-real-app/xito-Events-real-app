...
export async function getAlbumCount(registeredDateTimeAD: string, albumType: string): Promise<number> {
  ...
  if (error) {
    console.error('Error counting album:', error);
    return 0;
  }
  return count || 0;
}
...
export async function addToAlbum(
  ...
  // Check count first
  const current = await getAlbumCount(registeredDateTimeAD, albumType);
  if (current >= MAX_ALBUM_PHOTOS) {
    return false;
  }
